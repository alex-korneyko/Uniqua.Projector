...
- No change a member makes is lost silently: a board's structural rules hold under simultaneous changes, and a change based on an outdated view is refused and explained rather than applied over someone else's.
...
- **Adding members to a board** — invitations are roadmap step 7. Until then every board has exactly one member, its owner, but the membership check is the real one and is exercised against a member who is not the owner.
...
### US-08: Not overwrite someone else's change
...
**Then** the system creates the board with that name, makes the account its board owner and only member, gives it three columns named To do, In progress and Done in that order, and opens it
...
**Then** the system refuses and tells them an account can own at most 50 boards, because each account's share of the product is bounded
...
**Then** the system lists every board they are a member of, marks the ones they own, and lists no other board
...
**Then** the system adds the column at the end of the board and shows it to them
...
**Given** a board member who is viewing a column, and another member who has since renamed that same column
**When** the first member saves their own new name for it
**Then** the system refuses it, tells them someone else renamed the column first, shows its current name, and keeps the name they typed so they can apply it again
...
**Then** the system refuses and tells them a board can hold at most 20 columns
...
**Then** the system refuses and tells them a board can hold at most 1,000 cards
...
**Then** the system shows exactly the characters that were typed, as plain text, and nothing in them is ever run or rendered as formatting

### AC-17 (US-04) — error

**Given** an account that has attempted 120 changes within the past minute — counting every attempt, whichever board it named and whether or not they belong to it
**When** they attempt a further change
**Then** the system refuses it, says plainly that changes are temporarily limited, tells them when they may continue, and records nothing — answering identically whichever board the change named, so the refusal reveals nothing about any board

### AC-18 (US-05) — happy path

**Given** a board member viewing a card
**When** they delete it and confirm
**Then** the system removes the card, the other cards in its column keep their relative order, and a member who then tries to change that card is told it no longer exists
...
**Given** the board owner viewing their board
**When** they choose to delete it and confirm by entering the board's name
**Then** the system deletes the board together with all its columns and cards, and from then on answers any request about that board, or anything that was on it, exactly as it answers for a board that never existed

### AC-21 (US-07) — happy path

**Given** an invited member of a board — a board member who is not its board owner
...
**Given** an invited member of a board
...
**Given** a board member who opened a card, and another member who has since changed that same card's title or description
**When** the first member saves their own change to that card
**Then** the system refuses it, tells them someone else changed the card first, shows them the card as it is now, and keeps the text they typed so they can apply it again

### AC-24 (US-08) — domain invariant

**Given** a board member whose view of a board's columns is outdated because another member has since added, reordered or deleted a column — a rename alone does not change the order
...
**Then** the system accepts both changes, because a change is refused as stale only when the very thing it changes was changed by someone else first
...
**Then** the system gives them exactly the same refusal it gives for a board that does not exist, reveals nothing of the board's content, and records nothing
...
**Then** the system refuses it exactly as if that column or card did not exist, and records nothing on either board
...
**Then** the system records nothing, presents the sign-in form, and does not treat the change as coming from that member

## 6. Non-functional requirements

| Aspect | Target | Measurement |
|---|---|---|
| Latency p95, opening a board holding 20 columns and 1,000 cards | ≤ 300 ms | server-side timing, sampled in the smoke run on the reference machine |
| Latency p95, a single change (add, rename, reorder, edit, delete) | ≤ 200 ms | server-side timing, sampled in the smoke run on the reference machine |
| Throughput | ≥ 50 changes/s across boards | smoke test on the reference machine — the 2-vCPU virtual machine on the self-hosted host named in the accounts-and-sessions spec §6; in CI a regression check only |
| Board invariants under simultaneous changes | 0 boards left with no column and 0 non-empty columns deleted, across 1,000 randomised pairs of simultaneous deletes and adds | integration test issuing the pairs concurrently against the real store |
...
| Per-account change rate | at most 120 changes per account per rolling minute; the 121st is refused and nothing is recorded | integration test against a controllable clock |
...
- **AuthZ/AuthN impact:** introduces the product's first authorisation boundary. Every read and change is answered only after checking that the signed-in account is a member of the board concerned, and before any other check but one: the per-account change limit (AC-17), which depends only on the account, counts every attempt whichever board it names, and refuses identically for any board, so it reveals nothing about one. The order is therefore: session, per-account limit, membership, then everything else; every column and card a request names is checked to belong to that same board. One capability is reserved to the board owner — renaming and deleting the board — and must be shown to refuse a board member who is not the owner, even though no such member can join until invitations exist.
- **Abuse cases:**
  - *Cross-board substitution* — naming another board's column or card through one's own board: refused as though the item did not exist, nothing recorded on either board (AC-26). The sharpest vector from the adversarial pass.
