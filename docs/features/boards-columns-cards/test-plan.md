...
| AC-20 happy path | the deletion dialog sends the typed name for the server to compare and returns to the list | component | Delete is always enabled, because the server compares the name (screens.md SCR-07); on success the list is shown without the board |
...
- A request body that is not the expected shape, or a value of the wrong type, sent by a non-member (AC-25, ADR 0014 lenient binding) → the non-member refusal, never a validation answer. Membership is decided before the body is judged. A body that is not JSON at all is the one exception: it is answered `request_malformed` before membership, identically for every board, so it reveals nothing (ADR 0014).
