...
| Failed sign-ins per request source | at most 20 in a 15-minute sliding window, reserved before the password is verified and released only on success, so the cap holds even against a client that never waits for its own delayed response; the 21st refuses with `accounts.sign_in_rate_limited` (429) without verifying a password; identical for a registered and an unregistered address | integration test against a controllable clock (ADR 0010 amendment, N-01) |
...
- **Abuse cases:**
  - *Password guessing*: progressive per-account delay (AC-12), plus a cap of 20 failed sign-ins per request source per 15-minute sliding window, reserved before the password is verified and released only on success, so a client that hangs up rather than waiting for its own delayed response — or several such clients running in parallel — still exhausts the cap and is refused with `accounts.sign_in_rate_limited` (429) before a further password is checked (ADR 0010 amendment, review 2026-09-22-2 N-01). Applies identically to a registered and an unregistered address (AC-05b). Accepted residual risk — an attempt distributed thinly across many accounts is not slowed by the per-account delay, and an attempt distributed thinly across many request sources is not slowed by the per-source cap either.
