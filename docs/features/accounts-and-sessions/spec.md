...
**Then** the system refuses it after a delay that grows with each additional failure, while a correct password is still accepted immediately, so that guessing becomes futile without the account ever becoming unusable to its owner — read as "never by anyone who does not share the owner's request source" (review 2026-09-23 P-01; the accepted residual for someone who does is recorded in §6.1); the count of consecutive failures returns to zero either on a correct password or after 15 minutes in which no failed attempt reached verification (review 2026-09-23 (third re-review) S-03: a 429 returned by `SignInRateLimit` before a password is even checked does not, by itself, keep the count alive)
...
| Progressive delay under guessing | 6th consecutive failure delayed ≥ 2 s; 10th ≥ 30 s; the delay grows with each further failure up to a ceiling of 5 minutes (reached at the 14th), which bounds how long one attempt can hold a connection; a correct password never delayed; the failure count returns to zero after 15 min in which no failed attempt reached verification | integration test against a controllable clock |
| Failed sign-ins per (request source, address) | at most 20 in a 15-minute sliding window, reserved before the password is verified and released only on success, so the cap holds even against a client that never waits for its own delayed response; the 21st against that same address refuses with `accounts.sign_in_rate_limited` (429) without verifying a password; identical for a registered and an unregistered address; a looser ceiling of 100 per request source across every address bounds rotation past the per-address cap; neither cap applies when the request source cannot be resolved | integration test against a controllable clock (ADR 0010 amendment, N-01; re-keyed, P-01) |
...
- **Abuse cases:**
  - *Password guessing*: progressive per-account delay (AC-12), plus a cap of 20 failed sign-ins per (request source, normalised email address) per 15-minute sliding window, reserved before the password is verified and released only on success, so a client that hangs up rather than waiting for its own delayed response — or several such clients running in parallel — still exhausts the cap and is refused with `accounts.sign_in_rate_limited` (429) before a further password is checked (ADR 0010 amendment, review 2026-09-22-2 N-01, re-keyed by review 2026-09-23 P-01). A looser ceiling of 100 failed sign-ins per request source across every address it has tried, in the same window, bounds a guesser who rotates across addresses from one source to route around the per-address cap. When the request source cannot be resolved (no `RemoteIpAddress`), neither cap is applied at all — keying on the literal "unknown" would give every such caller one shared budget, which is worse than not capping them — and the gap is logged twice: an error that the request source itself could not be resolved (shared with *Spam account creation* below), and, for the sign-in path specifically, a further warning that both sign-in caps were skipped for it. Both caps key on the same normalised request source described under *Spam account creation* below, including its own tracked-source ceiling (review 2026-09-23 P-02) — but the per-address cap's own tracked-pair ceiling counts **(source, address) pairs, shared by every source**, not distinct sources: about 1,000 sources with 100 addresses each already fill it within one 15-minute window. Past it a new pair goes untracked and the per-address cap is skipped for it, falling back to the looser per-source figure of 100 above (review 2026-09-23 (third re-review) S-02). Applies identically to a registered and an unregistered address (AC-05b). Accepted residual risk — an attempt distributed thinly across many accounts is not slowed by the per-account delay; an attempt distributed thinly across many request sources is not slowed by the per-source ceiling either; and, because the cap is now keyed by address rather than by source alone, a guesser who shares the owner's request source and targets the owner's own address specifically, or a source that has reached its own per-source ceiling, can still refuse the owner for up to 15 minutes (review 2026-09-23 P-01) — narrower than before, since it no longer reaches a correct password for any other address from that same source.
  - *Account enumeration through the registration form*: accepted and deliberate. The form states plainly that an address is already registered (AC-03) and that a display name is taken (AC-11b), because an ambiguous form defeats unattended registration, which is load-bearing; the display-name refusal additionally reveals which names are in use, accepted for the same reason. The sign-in form adds no second oracle — an address no account was registered with is refused in the same words and in a comparable time as a wrong password (AC-05, AC-05b). Accepted residual risk — because an unregistered address still costs a full password verification, the sign-in form is a cheap way to consume processor time; the *Password guessing* caps above bound the volume from one (source, address) pair or one source, but not a spray thin enough to stay under both, and the spam-registration rate limit below covers registration only, not sign-in (review 2026-09-23 P-03). The comparable-time guarantee for an unregistered address (AC-05b) rests on an in-process counter (`IUnknownAddressAttempts`, sad §5) that carries two further accepted limits, recorded here because they are otherwise stated only in a code comment (review 2026-09-22-2 N-03): a restart clears every count, so for the first few attempts after a restart an unregistered address answers faster than a registered one already under guessing; and past 100,000 distinct addresses tracked inside one reset window, a further new address goes uncounted and answers at the untracked, fastest rate. Both only matter to someone spraying addresses at a scale the registration-form enumeration above already serves better.
  - *A wrong password against a registered address costs more round trips than against an unregistered one* (AC-05b): the comparable-time guarantee above holds on wall-clock time, but not on database round trips. A registered address costs three — `FindByEmailAsync`, the by-id read `VerifyPasswordAsync` makes, and the `ExecuteUpdateAsync` `RecordFailureAsync` makes on the uncontended path — against one, `FindByEmailAsync` alone, for an unregistered address, which then only pays a dummy password hash with no further database call. Accepted and recorded (review 2026-09-23 Q-10, N-10 remainder); no code change — the wall-clock comparability AC-05b actually requires is unaffected, since the hash, not the round trip count, dominates the cost either way.
  ...
  - *Spam account creation*: rate limit — no more than 5 registrations per minute per request source, where a **request source** is the client address as reported by this instance's own reverse proxy. That report is trusted only when the request arrives from the proxy itself: trusting it unconditionally would let anyone forge the key and make the limit decorative, and ignoring it entirely would key every visitor to the proxy's own address and close registration for the whole world after five accounts. The address is normalised before it becomes a key: an IPv4-mapped IPv6 address (`::ffff:a.b.c.d`) is keyed by its plain IPv4 form, and any other IPv6 address is keyed by its /64 prefix — the block an ISP hands one customer — rather than by the full address, so rotating within it does not buy a fresh limit (review 2026-09-23 P-02). Accepted residual risk — people behind one shared outbound address, or one shared /64, share a limit; and past 100,000 distinct sources tracked inside one 1-minute window — or, for the sign-in per-source cap above, one 15-minute window; the sign-in per-address cap's own 100,000-entry ceiling counts (source, address) pairs instead, per the fallback described above (review 2026-09-23 (third re-review) S-02) — a further new source is not tracked at all and its attempt is permitted uncounted, logged as an error — the same accepted shape of limit `IUnknownAddressAttempts` already carries above, at the same 100,000 figure. Both only matter to someone flooding distinct sources (IPv6 rotation among them) at a scale this instance's own reverse-proxy trust already makes hard to sustain.
...
- [x] What normalisation applies to an email address at registration versus at sign-in (letter case, surrounding whitespace)? **Resolved 2026-09-21 during `/sdd:data-model`, as the standing default said:** the application normalises — trim, then invariant upper-case, by ASP.NET Core Identity's normalizer — and registration and sign-in call the identical code path. The result is stored in `NormalizedEmail`, which carries the unique index that makes AC-03 true; no column collation is overridden, so the database is never the thing that decides who is a duplicate. See `data-model.md` § `AspNetUsers`. — owner: Alex Korneiko
- [x] ADR 0003 records "lockout after repeated attempts" among the positive consequences of its chosen authentication mechanism, but AC-12 and §6 commit to a progressive delay instead, and no superseding decision is written down. **Resolved 2026-09-21 by [ADR 0010](adr/0010-replace-account-lockout-with-a-progressive-per-account-delay.md):** ADR 0010 supersedes only that one consequence of ADR 0003 — lockout is replaced by a progressive per-account delay plus the per-source cap (ADR 0010 amendment, review 2026-09-22-2 N-01) — and ADR 0003 now carries a "see ADR 0010" note where the superseded consequence was recorded. — owner: Alex Korneiko
- [x] Which screen does a visitor who is not signed in see first? **Resolved 2026-09-22 by the owner: the sign-in form, for every visitor**, with the way to registration ("No account yet? Create one") inside the same card. Most arrivals already have an account, and AC-07, AC-07b and AC-10 all end "presents the sign-in form"; a stranger from the public link still reaches registration unaided and in one click, which is all AC-01 and the first goal in §2 require. This replaces the implementation's earlier choice of registration first for a first-time arrival, which no criterion asked for. — owner: Alex Korneiko
- [ ] Deferred from review 2026-09-22 (`_review/review-2026-09-22.md`, R-12): AC-01 and AC-04 promise that the visitor is then shown their own display name, but the registration and sign-in screen tests stop at "no alert is showing" and never follow the 201 → `/me` → display-name path in the rendered shell. Default now: the backend endpoint tests and the shell's display-name test cover the two halves separately. — owner: Alex Korneiko, due: before the next UI feature's `/sdd:review`
- [ ] Deferred from review 2026-09-22 (R-24): CLAUDE.md says migrations run on startup in development, but only the integration-test factory calls `MigrateAsync`, so a fresh development database has no tables until `dotnet ef database update` is run by hand. Default now: apply migrations by hand locally. — owner: Alex Korneiko, due: before the next feature adds a migration
- [ ] Deferred from review 2026-09-22 (R-27): on the registration screen, a field refusal is not linked to its input through `aria-describedby`, so a screen reader can lose the reason when focus moves to the invalid field. Default now: the refusal is announced through `role="alert"`. — owner: Alex Korneiko, due: before the next UI feature's `/sdd:review`
- [ ] Deferred from review 2026-09-23 (`_review/review-2026-09-23-4.md`, W-01): openapi's sign-in 401 description and both `email` schemas say an address longer than `maxLength: 256` is refused, but the sign-in guard and registration measure it after trimming surrounding whitespace, so a padded 257-character value is accepted. Default now: the code's trimmed rule, as sad flow 4 states it, is the authority; the contract is stricter than the code. — owner: Alex Korneiko, due: before the next feature's `/sdd:review`
- [ ] Deferred from review 2026-09-23 (W-02): the SPA fallback (`Program.cs` `MapFallbackToFile`) matches GET and HEAD on every path without an extension, `/api` included, so `GET /api/v1/sessions/current` answers 200 `text/html` instead of the 405 `api.request_rejected` the contract's `RequestRejected` says reaches the app. Non-safe wrong methods do get the 405. No test sends a real-route 405. Default now: GET/HEAD on a non-GET API path falls through to the SPA. — owner: Alex Korneiko, due: before the next feature's `/sdd:review`
- [ ] Deferred from review 2026-09-23 (W-03): `contracts/api-sync-report.md` was not reconciled with the implemented contract: its self-check still counts one open question, it says no error registry exists (`AccountProblems.cs` holds 14 codes), its tables miss the 400/413/429/4XX responses, and the Spectral result dates from 2026-09-21. Default now: openapi.yaml is authoritative; the report is historical. — owner: Alex Korneiko, due: before the next feature's `/sdd:api`
- [ ] Deferred from review 2026-09-23 (W-04): the readable `XSRF-TOKEN` cookie the contract declares on GET `/accounts/me` (200 and 401) is tested only on `/health` and by name; no test asserts it is not HttpOnly, is Secure and is SameSite=Strict, though every state-changing call from the SPA depends on reading it. Default now: covered only by the SPA working end to end. — owner: Alex Korneiko, due: before the next feature's `/sdd:review`
- [ ] Deferred from review 2026-09-23 (W-05): the web tests for AC-02 and AC-01b (`RegisterScreen.test.tsx`) stub refusal `detail` text the server never sends ("at least 8 characters" against the real "between 8 and 128"), and a comment misquotes the contract's 429 example. Default now: the tests prove the screen renders whatever detail arrives. — owner: Alex Korneiko, due: before the next UI feature's `/sdd:review`
- [ ] Deferred from review 2026-09-23 (W-06): two test comments (`SignInTests.cs`, `GuessingDelayTests.cs`) and `IUnknownAddressAttempts.cs` still quote the superseded reset rule ("no attempt is made", "quiet minutes") as AC-12's words. Default now: AC-12 in §5 is the authority. — owner: Alex Korneiko, due: before the next feature's `/sdd:review`
- [ ] Deferred from review 2026-09-23 (X-01): V-03's `IsWithinInterval` treats any negative elapsed time as due, so under concurrency a thread whose `now` was read a few ticks before another thread stamped the throttle runs an extra full prune and writes an extra ceiling line, weakening T-01's once-per-interval throttle during a flood at capacity. All throttle tests are single-threaded. Default now: accepted — extra scans occur only while a limiter sits at capacity. — owner: Alex Korneiko, due: before the next feature's `/sdd:review`
- [ ] Deferred from review 2026-09-23 (X-03): `.gitignore:54,55,62,86` (`[Tt]est[Rr]esult*/`, `[Bb]uild[Ll]og.*`, `TestResults/`, `coverage/`) and `src/Uniqua.Projector.Web/.gitignore:2,11` (`logs`, `dist`) are unanchored and would ignore source or docs folders of those names. Default now: no such folder exists; `git ls-files -ci --exclude-standard` lists nothing. — owner: Alex Korneiko, due: before a source folder with one of those names is added
